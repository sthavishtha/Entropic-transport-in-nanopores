...
NTRA = 5e3; NTIME = 1000; NHIST = 2; DT = 0.001;
...
%Bacterium velocity
V_bact = 1;             % Is a hundredth of the characteristic lengthscale L  
...
k
    ...
    for j = 1:NTIME
        ...
        % Difference equation
%       Phi = 2*pi*rand(NTRA,1);
        Phi(:, j + 1) = Phi(:, j) + sqrt(Da*DT)*randn(NTRA, 1);
        x(:, j + 1) = x(:, j) + fx*DT + V_bact*DT*cos(Phi(:, j + 1)) + sqrt(Dx*DT)*randn(NTRA, 1);
        y(:, j + 1) = y(:, j) + V_bact*DT*sin(Phi(:, j + 1)) + sqrt(Dy*DT)*randn(NTRA, 1);
        ...
        Deffa(j+1, k) = var(Phi(not(AbsParts), j + 1))/(j*DT);
        Var_x(j+1, k) = var(x(not(AbsParts), j + 1));
        Var_y(j+1, k) = var(y(not(AbsParts), j + 1));
        Var_a(j+1, k) = var(Phi(not(AbsParts), j + 1));
...
toc
    ...
    Deffa_avg = mean(Deffa, 2);
    Varx_avg = mean(Var_x, 2);
    Vary_avg = mean(Var_y, 2);
    Vara_avg = mean(Var_a, 2);
...
lname = legend('Variance x', 'Variance y', 'Variance angular');
